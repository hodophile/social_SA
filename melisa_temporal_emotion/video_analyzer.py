...
# --- shim so `src.*` (vendored Melisa POC) imports resolve --------------
import sys as _sys
from pathlib import Path as _Path

_MELISA_ROOT = _Path(__file__).resolve().parent.parent / "melisa_poc"
if str(_MELISA_ROOT) not in _sys.path:
    _sys.path.insert(0, str(_MELISA_ROOT))
# -------------------------------------------------------------------------
...
class VideoAnalysisBundle:
    ...
    # NEW: per-frame 8-emotion distributions
    frame_emotions: list = field(default_factory=list)
...
class VideoAnalyzer:
    ...
    def __init__(
        ...
        *,
        ...
        # NEW: per-frame 8-emotion classifier
    ...
    def analyze(
        ...
        *,
        ...
        try:
            ...
            frame_emotions_all: list[dict] = []  # NEW
            ...
            for idx, frame_path in enumerate(frame_paths):
                ...
                try:
                    ...
                    # NEW: 8-emotion distribution for this frame
                    try:
                        frame_emotions_all.append(
                            self._emotion_net.predict_emotion(
                                np.asarray(loaded_frames[idx])
                            )
                        )
                    except Exception as exc:  # noqa: BLE001
                        warnings.append(f"frame[{idx}] emotion-net failed: {exc}")
            ...
            return VideoAnalysisBundle(
                ...
                warnings=warnings,
                overall=overall,
                frame_emotions=frame_emotions_all,  # NEW
            )
        finally:
