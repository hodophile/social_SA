...
Each script runs the full pipeline (preprocess → PLM → structured JSON →
harm scoring → downstream action) against a sample asset in `assets/`.
All tests use the **mock PLM**, so no GPU or checkpoint is required.
...
The examples default to `PLM1B(mock=True)`. To use a real checkpoint
(`PLM1B._load_model` is implemented via HuggingFace transformers,
